...
class ProjectItem extends Component {
  ...
  render() {
    ...
    current.active ? itemClass.push("active") : itemClass.push("false");
    !current.visible && itemClass.push("hidden");

    function asshole() {
      itemClass.push("archived");
    }
    ...
    return (
      <div className={itemClass.join(" ")}>
        ...
        <button onClick={props.toggleTimer}>
          {current.active ? pause : resume}
        </button>
        <button onClick={() => {props.archive();asshole();}}>
          archive
        </button>
      </div>
    );
